...
if [ ! -z "${scratch_path}" ]; then
    ...
    cp -R "${datasets_path}/Silva_Nr99_Filtered" "${scratch_path}/${run_id}/datasets"
    datasets_path="${scratch_path}/${run_id}/datasets"
fi

# Train the model
${command_prefix} ${python_tf} ./scripts/pretraining/pretrain_dnabert.py \
    --wandb-name silva-nr99-filtered-64d-150bp \
    --sequences-fasta-db "${datasets_path}/Silva_Nr99_Filtered/Silva_Nr99_Filtered.fasta.db" \
    $@
