...
class DnaSampleVeeGan(VeeGan):
    def __init__(self, generator, discriminator, reconstructor, encoder, batch_size, subsample_size, encoder_batch_size=512):
        super().__init__(generator, discriminator)
        self.encoder = encoder
        self.encoder.trainable = False
        self.encoder_batch_size = encoder_batch_size
...
class DnaSampleConditionalVeeGan(ConditionalVeeGan):
    ...
    def encode_data(self, data):
        ...
        for i in range(0, self.batch_size*self.subsample_size, self.encoder_batch_size):
            encoded.append(tf.stop_gradient(self.encoder(flat_data[i:i+self.encoder_batch_size])))
        return (tf.reshape(tf.concat(encoded, axis=0), (self.batch_size, self.subsample_size, -1)), data[1])
