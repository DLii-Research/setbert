...
class PersistentDnaBertTopDownTaxonomyModel(dcs.module.Wandb.PersistentObject[TopDownTaxonomyClassificationModel[DnaBertEncoderModel]]):
    ...
    def load(self):
        return load_model(self.path("model"), TopDownTaxonomyClassificationModel[DnaBertEncoderModel])
...
def data_generators(
    ...
    sequence_length: int,
    kmer: int,
    taxonomy_tokenizer: TopDownTaxonomyTokenizer
):
    ...
    generator_pipeline = [
        ...
        lambda taxonomy_labels: dict(
            taxonomy_labels=np.array(recursive_map(
                lambda sequence: taxonomy_tokenizer.tokenize_label(sequence)[-1],
                taxonomy_labels))),
...
def main(context: dcs.Context):
    ...
    model = PersistentDnaBertTopDownTaxonomyModel()
