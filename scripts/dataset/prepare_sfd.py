...
def main(context: dcs.Context):
    ...
    output_path = config.output_path / "SFD"
    ...
    otu_list_path = config.input_path / "/home/shared/walker_lab/digitalocean/Alex_SFD/shared_list/221205_cmfp.trim.contigs.pcr.good.unique.good.filter.unique.precluster.denovo.vsearch.asv.list"
    otu_shared_path = config.input_path / "/home/shared/walker_lab/digitalocean/Alex_SFD/shared_list/221205_cmfp.trim.contigs.pcr.good.unique.good.filter.unique.precluster.denovo.vsearch.asv.shared"

    # Create FASTA DB
    factory = fasta.FastaDbFactory(output_path / "SFD.fasta.db")
    for entry in tqdm(map(clean_entry, fasta.entries(fasta_path))):
        if len(entry) < 250:
            continue
        factory.write_entry(entry)
    factory.close()
    fasta_db = fasta.FastaDb(output_path / "SFD.fasta.db")
    ...
    factory = fasta.FastaIndexDbFactory(output_path / "SFD.fasta.index.db")
    ...
    factory.close()
    index_db = fasta.FastaIndexDb(output_path / "SFD.fasta.index.db")
    ...
    factory = sample.SampleMappingDbFactory(output_path / "SFD.fasta.mapping.db")
